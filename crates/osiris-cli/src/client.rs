...
/// certificate(s) are added as trusted roots in addition to the operating
/// system's native roots (rustls with native roots); without it the default
...
pub fn build_client(
    ...
    if certs.is_empty() {
        return Err(format!(
            "--ca-cert {} contains no certificate",
            path.display()
        ));
    }
