...
pub fn dispatch(
    ...
    Ok(ResponseOutcome::Rejected {
        reason: "Active Actions milestone not yet shipped — see ARCHITECTURE.md §13/§29"
            .to_string(),
    })
}
...
mod tests {
    ...
    use osiris_schema::{
        Category, DnsRef, EventType, HostRef, ProcessKey, ProcessRef, Severity, Source,
        SCHEMA_VERSION,
    };
    ...
    fn harness() -> Harness {
        ...
        Harness {
            ...
            evidence: SqliteEvidenceStore::open(dir.path().join("evidence.db").to_str().unwrap())
                .unwrap(),
            links: SqliteEvidenceIncidentLinks::open(dir.path().join("links.db").to_str().unwrap())
                .unwrap(),
            _dir: dir,
        }
    }
    ...
    fn dry_run_on_a_destructive_action_returns_a_preview_and_touches_nothing() {
        ...
        assert!(
            h.evidence.list().unwrap().is_empty(),
            "dry run must not create evidence"
        );
    }
    ...
    fn collect_evidence_executes_and_persists_a_record() {
        ...
        let outcome = dispatch(&req, &h.storage, &h.evidence, &h.links).unwrap();
        let ResponseOutcome::EvidenceCollected {
            evidence_id,
            event_count,
            truncated,
        } = outcome
        ...
        let stored = h
            ...
            .get(evidence_id)
            .unwrap()
            .expect("evidence must be persisted");
    ...
    fn collect_evidence_with_zero_matching_events_still_succeeds() {
        ...
        let req = ResponseRequest {
            action: ResponseActionKind::CollectEvidence,
            target: EntityRef::Domain {
                name: "never-seen.example".to_string(),
            },
    ...
    fn collect_evidence_links_to_an_incident_when_one_is_given() {
        ...
        assert_eq!(
            h.links.evidence_ids_for_incident(incident_id).unwrap(),
            vec![evidence_id]
        );
    }
    ...
    fn evidence_collected_reports_a_small_untruncated_event_count() {
        ...
        for i in 0..3u64 {
            ...
            e.category = Category::Dns;
            e.dns = Some(DnsRef {
                query: "small-batch.example".to_string(),
                qtype: "A".to_string(),
                response_ips: vec![],
                ttl: None,
            });
        ...
        let req = ResponseRequest {
            action: ResponseActionKind::CollectEvidence,
            target: EntityRef::Domain {
                name: "small-batch.example".to_string(),
            },
    ...
    fn every_destructive_action_is_rejected_when_not_a_dry_run() {
        ...
        assert!(
            h.evidence.list().unwrap().is_empty(),
            "no destructive action may create evidence"
        );
    }
}
